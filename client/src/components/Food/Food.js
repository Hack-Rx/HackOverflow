...
class Food extends React.Component {
  ...
  /*
  name getFood,
  work : Send todays date and retrieve food for the user.
  */
  getFood = async () => {
    await submitForm(
      undefined,
 ...
 /*
  ...
  work : Update the date to the next day and get new food data to reflect that.
  */
  ...
  work : Update the date to the previous day and get new food data to reflect that.
  */
  previousDateOnClick = async () => {
   ...
   /*
  name createFood,
  work : Creates the food html based on state data and mealtype.
  */
  createFood = (mealType) => {
  ...
  /*
  name editFood,
  work : Send the user to the edit page to edit their food item.
  */
  editFood = (url) => this.props.history.push(url);

  /*
  name deleteFood,
  work : Delete a food item that the user wants to delete.
  */
  deleteFood = async (id) => {
    ...
    return (
      <div id="foodContainer">
        <div id="calorieSummary">
          ...
          {this.state.calorieGoal - this.state.caloriesConsumedValue<0 ? 
          <div>
            <h2>You have exeeded the require Calorie Intake. Let's get them back on track by some exercises. Choose any one option</h2>
            <br></br>
            <h2>Pushups: {pushups} reps </h2> <br></br>
          <h2>Swimming: {swimming} mins </h2> <br></br>
          <h2>Jogging: {jog} miles </h2> <br></br>
</div>
          :null}
          <span className="divider"></span>
        </div>
